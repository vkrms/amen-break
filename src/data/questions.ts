// cbt protocol
export const questions = [
    'The thought',
    'How does it make me feel?',
    'How does it make me act?',
    'What is the outcome of that action?',
    'Is the thought true?',
    'Is it absolutely true?',
    'How would I feel if I didn’t have that thought? Imagine it',
    'How would I act if I didn’t have that thought?',
    'What is the opposite of that thought? Dwell on it.',
]

export const questionsRu = [
    'Мысль',
    'Что я чувствую?',
    'Как я действую?',
    'К чему это ведёт?',
    'Эта мысль - это прадва?',
    'Сто проц?',
    'Как бы я себя чувствовал без этой мысли? Представь.',
    'Как бы я себя вёл не будь этой мысли?',
    'Как бы звучала противоположная мысль? Задумайся.',
]

export const slugs = [
    'thought',
    'feeling',
    'action',
    'outcome',
    'true',
    'absolutely-true',
    'no-thought',
    'no-action',
    'opposite',
]
